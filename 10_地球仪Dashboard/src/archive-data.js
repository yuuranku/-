...
const abnormalities = [
  ...
  ['1943.02.09', '寒区热钻供电缺口', '战时寒区热钻试验场', '热钻耗电长期超出供电台账，三班电工的值勤表都不认领当班开钻。', '职位错置', 'warning'],
  ['1951.03.14', '迪蒙·迪维尔海岸多绘', '迪蒙·迪维尔海岸航测段', '法方独立航片在英美底稿的同一坐标上多绘出一条内陆谷口。', '地理闭合失败', 'observed'],
  ['1956.11.20', '黑湖声呐回声三读', '黑湖水气交界', '一次换能器投放的撞击声，在声呐纸带上比投放动作早约三秒出现。', '证据互斥', 'warning'],
  ['1957.07.01', '联合索引里程缺段', '国际地球物理年联合索引区', '归一坐标后，同一条测线的里程在三国记录里同时短少了同一段。', '行程缺段', 'observed'],
  ['1959.12.01', '议定书双附件', '南极条约签署地', '公开条约附件与秘密议定书附件对同一批入口给出互斥管辖，两份都盖生效章。', '地理闭合失败', 'warning'],
  ['1962.10.24', '多语种确认广播', '民用短波监听站', '一段多语种“确认安全”广播的名单里，既有已列失踪、也有尚未派出的人。', '人数不闭合', 'critical'],
  ['1963.08.19', '黑湖血样病历互斥', '南极回返者接收医院', '同一名回返者两家医院的血型与病历互斥，两份都通过校验，输血与家属通知无法处理。', '人员重叠', 'critical'],
  ['六十年代中期', '元首回响封存令', '某参与国本土机关', '同国两个无通联机关同期发出措辞一致的封存令，最高层简报记着一次从未发生的南极汇报。', '元首回响', 'warning'],
