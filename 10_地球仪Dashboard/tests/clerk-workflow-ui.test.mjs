...
test('mailbox is a formal system inbox with an administrator-only announcement composer', () => {
  assert.match(html, /aria-label="邮箱"/);
  assert.match(html, /archive-inbox\.svg/);
  assert.match(workspace, /data-mailbox-compose/);
  assert.match(workspace, /data-mailbox-recipient/);
  assert.match(workspace, /client\.sendAnnouncement\(/);
  assert.match(workspace, /PALIS 档案管理处/);
  assert.match(workspace, /notification\.kind === 'announcement'/);
});
