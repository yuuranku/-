...
const normalizeError = (error, context) => {
  ...
  return new ArchiveWorkflowError(`${context}: ${error?.message || 'unknown error'}`, {
    code: error?.code || 'archive_workflow_error',
    cause: error,
...
export const createArchiveWorkflowClient = (supabase) => {
  ...
  const listTemplates = () =>
    unwrap(
      supabase
        ...
        .select('id,code,category,title,schema,active')
        .eq('active', true)
  ...
  const saveDraft = async (draft) => {
    ...
    const data = await unwrap(
      supabase
        ...
        .eq('id', requireId(draft.id, 'draftId'))
        .eq('owner_id', ownerId)
  ...
  const submitDraft = (draftId, ownerId) =>
    unwrap(
      supabase
        .from('archive_contributions')
  ...
  const reviewSubmission = (submissionId, { decision, message }) => {
    ...
    return unwrap(
      supabase.rpc('review_archive_submission', {
  ...
  const publishContribution = (submissionId, registration) =>
    unwrap(
  ...
  const listNotifications = (recipientId) =>
    unwrap(
      supabase
        ...
        .select('*,contribution:archive_contributions(id,title,status,archive_id)')
        .eq('recipient_id', requireId(recipientId, 'recipientId'))
        .order('created_at', { ascending: false })
        .limit(100),
      'Unable to load notifications',
    );

  const markNotificationRead = (notificationId, recipientId) =>
    unwrap(
      supabase
        .from('archive_notifications')
        .update({ read_at: new Date().toISOString() })
        .eq('id', requireId(notificationId, 'notificationId'))
        .eq('recipient_id', requireId(recipientId, 'recipientId'))
        .select('*')
        .single(),
  ...
  const listArchiveContributions = (archiveId) =>
    unwrap(
      supabase.rpc('list_public_archive_contributions', {
  ...
  const listArchiveReferences = (archiveId) =>
    unwrap(
      supabase
        ...
        .eq('target_archive_id', requireId(archiveId, 'archiveId'))
        .not('source_archive_id', 'is', null)
        .order('created_at', { ascending: false }),
  ...
  const uploadAttachment = async (contributionId, ownerId, file) => {
    ...
    if (!file?.name || !Number.isFinite(file?.size) || file.size <= 0 || file.size > 5 * 1024 * 1024) {
      throw new ArchiveWorkflowError('Attachment must be between 1 byte and 5MB', {
        code: 'invalid_attachment',
      });
    }
    if (!supabase.storage?.from) {
      throw new ArchiveWorkflowError('Attachment storage is unavailable', {
    ...
    const uploaded = await unwrap(
      supabase.storage.from('archive-attachments').upload(storagePath, file, {
        cacheControl: '3600',
