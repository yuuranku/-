...
const legacyEventDate = (title) => {
  const match = /^(\d{4})\s*(?:年|[./-])\s*(\d{1,2})\s*(?:月|[./-])\s*(\d{1,2})\s*(?:日)?/.exec(String(title ?? '').trim());
  if (!match) return '';
  const [, year, month, day] = match;
  return `${year}.${month.padStart(2, '0')}.${day.padStart(2, '0')}`;
};

const categoryProjection = (category, indexData, sequenceNumber, title = '') => {
  switch (category) {
    case 'country':
      return {
    ...
    case 'event': {
      const eventDate = indexData.startDate || legacyEventDate(title);
      return {
        year: eventDate,
        eventDate,
...
export const projectPublishedArchive = (archive) => {
  ...
  return {
    ...
    ...categoryProjection(category, indexData, sequenceNumber, title),
  };
};
