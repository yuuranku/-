...
const renderMarkedTitle = (document, value) => renderInlineText(
  value,
  document?.inlineMarks?.title
    || document?.inlineMarks?.hero
    || document?.inlineMarks?.['amendment:title']
    || [],
);
...
const renderLegacyInlineCitation = (value, references = []) => String(value ?? '')
  .split(/(〔[^〕\r\n]+〕)/g)
  .map((part) => {
    const citation = /^〔([^〕\r\n]+)〕$/.exec(part);
...
const renderInlineCitation = (value, references = [], marks = []) => {
  const text = String(value ?? '');
  let offset = 0;
  const marksForPart = (start, end) => (Array.isArray(marks) ? marks : [])
    .map((mark) => ({
      ...mark,
      start: Math.max(0, Number(mark?.start) - start),
      end: Math.min(end, Number(mark?.end)) - start,
    }))
  ...
  return text
    ...
    .map((part) => {
      ...
      offset += part.length;
      const citation = /^\u3014([^\u3015\r\n]+)\u3015$/.exec(part);
      const content = renderInlineText(part, marksForPart(start, offset));
      ...
      return `<button type="button" class="archive-formal-reference" data-open-archive-reference="${escapeHtml(target)}">${content}</button>`;
    })
    .join('');
};
...
const documentFieldRows = (document, keys) => keys
  .map((key) => ({
    ...
    value: documentFieldValue(document, key),
    marks: document.inlineMarks?.[key] || [],
  }))
...
const customEntries = (document) => {
  ...
  Object.entries(document.values).forEach(([key, value]) => {
    ...
    if (!match) return;
    const entry = entries.get(match[1]) || { id: match[1], label: '', value: '', labelKey: '', valueKey: '' };
    entry[match[2] === 'label' || match[2] === 'title' ? 'label' : 'value'] = visibleValue(value);
    entry[match[2] === 'label' || match[2] === 'title' ? 'labelKey' : 'valueKey'] = key;
  ...
  return [...entries.values()]
    ...
    .map((entry) => ({
      ...
      value: entry.value,
      marks: document.inlineMarks?.[entry.valueKey] || [],
    }));
};
    ...
    return `
      <section class="archive-formal-section record-chapter archive-formal-section--${escapeHtml(String(section.id).replaceAll(/[^a-z0-9_-]/gi, ''))}" data-formal-section="${escapeHtml(section.id)}">
        ...
        ${visibleValue(section.prose)
          ? `<div class="record-copy"><p>${renderInlineCitation(section.prose, references, section.marks || [])}</p></div>`
          : `<dl class="record-fields">
              ${rows.map((row) => `
                <div>
                  <dt>${escapeHtml(row.label)}</dt>
                  <dd>${renderInlineCitation(row.value, references, row.marks)}</dd>
                </div>
              `).join('')}
            </dl>`}
      </section>
  ...
  return `
    <header class="chain-mast">
      <div>
        ...
        <h2>${renderMarkedTitle(document, title)}</h2>
      </div>
...
const renderOrganizationDossier = (document) => {
  ...
  return `
    <div class="chain-columns">
      ...
      <dl class="record-fields">
        ${rows.map((row) => `
          <div><dt>${escapeHtml(row.label)}</dt><dd>${renderInlineCitation(row.value, references, row.marks)}</dd></div>
        `).join('')}
      </dl>
    </div>
  `;
...
const renderCountryArchive = (document, archive, version, preview) => {
  ...
  return `
    ...
    <div class="registry-sheet" data-formal-section="country-dossier">
      <aside><span>${escapeHtml(code)}</span>${renderCountryFlag(document, title)}<b>${renderMarkedTitle(document, title)}</b><i>PALIS / STATE REGISTRY</i></aside>
...
const renderStationArchive = (document, archive, version, preview) => {
  ...
  return `
    <header class="station-log-mast">
      <div><p class="dialog-meta">STATION OPERATIONS LOG</p><h2>${renderMarkedTitle(document, title)}</h2></div>
    ...
    <div data-formal-section="station-dossier">
      <dl class="station-log-grid">
        ${identityRows.map((row) => `<div><dt>${escapeHtml(row.label)}</dt><dd>${renderInlineCitation(row.value, references, row.marks)}</dd></div>`).join('')}
      </dl>
    </div>
  `;
};
      ...
      <div class="record-prose-flow">
        ${overview || coordinates ? `
          <section class="record-chapter record-chapter--station-overview">
            ...
            <div class="record-copy">
              ...
              ${overview ? `<p>${renderInlineCitation(overview, references, document.inlineMarks?.stationOverview || [])}</p>` : ''}
            </div>
          </section>
        ` : ''}
        ${rosterEntries.map((entry, index) => `
          <section class="record-chapter record-chapter--station-roster">
            ...
            <ul><li>${renderInlineCitation(entry.value, references, entry.marks)}</li></ul>
          </section>
        `).join('')}
      </div>
    </section>
...
const renderEntranceArchive = (document, archive, version, preview) => {
  ...
  return `
    <header class="descent-mast">
      <p class="dialog-meta">DESCENT CHART / FIELD DATUM</p><h2>${renderMarkedTitle(document, title)}</h2>${registrationStamp(version, preview)}
    </header>
...
const renderEcologyArchive = (document, archive, version, preview) => {
  ...
  return `
    <header class="strata-mast">
      <div><p class="dialog-meta">STRATA PROFILE / ${escapeHtml(code)}</p><h2>${renderMarkedTitle(document, title)}</h2></div>
      ${registrationStamp(version, preview)}
    </header>
...
const titleWithoutRepeatedEventDate = (title, eventDate) => {
  const source = visibleValue(title);
  const normalizedDate = visibleValue(eventDate);
  if (!source || !normalizedDate) return source;
  const leadingDate = normalizedDate.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return source.replace(new RegExp(`^${leadingDate}\\s*[／/]\\s*`), '');
};
  ...
  return `
    <header class="reel-mast">
      <div>
        ...
        <h2>${escapeHtml(eventDate ? `${eventDate} / ${titleWithoutRepeatedEventDate(title, eventDate)}` : title)}</h2>
      </div>
  ...
  <header class="archive-formal-document__mast archive-formal-document__mast--species specimen-mast">
    <div>
      <p>SPECIMEN &amp; TAXONOMIC PLATE / ${escapeHtml(document.templateCode)}</p>
          <h2>${renderMarkedTitle(document, document.title || '\u672a\u547d\u540d\u6807\u672c')}</h2>
    </div>
    ...
    <div class="specimen-layout" data-formal-section="species-dossier">
      <figure>
        ...
        <figcaption>
          ...
          <b>${renderMarkedTitle(document, document.title || '\u672a\u547d\u540d\u6807\u672c')}</b>
    ...
    <article class="archive-record-amendment archive-record-amendment--formal" data-amendment-for="${escapeHtml(targetId)}" data-amendment-id="${escapeHtml(contribution.id)}">
      <header>
        <div>
          ...
          <h5>${renderMarkedTitle(document, title)}</h5>
        </div>
