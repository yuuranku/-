...
test('published contribution ledger shows attribution, history, references and registration stamp', () => {
  ...
  assert.match(markup, /data-open-amendment-history="record-5"/);
  assert.doesNotMatch(markup, /data-amendment-for="record-1"/);
});

test('amendment rendering keeps references from the current version when the amendment has no references', () => {
  const { currentVersion } = buildAmendmentTimeline(
    {
      version_label: '0.1',
      content: {
        schemaVersion: 2,
        values: { body: '原始正文' },
        references: [{ archiveId: 'archive-1', code: 'O01', label: '原始档案' }],
      },
    },
    [{
      id: 'amendment-1',
  ...
  assert.deepEqual(currentVersion.content.references, [
    { archiveId: 'archive-1', code: 'O01', label: '原始档案' },
  ]);
});
