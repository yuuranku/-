...
test(
  'slash reference picker lists available archives before a clerk enters a query',
  { timeout: 60_000 },
  async (t) => {
    const { page } = await openLocalAdminBrowser(t);
    const fixture = await seedClerkAndReference(page);
    ...
    await setValue(page, '[data-native-custom-entry] [data-native-custom-content]', '/');
...
test(
  'inline format buttons apply marks directly inside the rich editor',
  { timeout: 60_000 },
  async (t) => {
    const { page } = await openLocalAdminBrowser(t);
    const fixture = await seedClerkAndReference(page);
    ...
    await page.$eval(field, (control) => {
      control.focus();
      control.textContent = '可公开资料与秘密线索';
    ...
    assert.equal(await page.$eval('.archive-editor-window:not([hidden]) [data-inline-format-status]', (output) => output.textContent), '已切换遮蔽');
    assert.deepEqual(
      await page.$eval(field, (control) => ({
      ...
      { redacted: '秘密线索', storage: '可公开资料与秘密线索' },
    );
    await clickControl(page, '.archive-editor-window:not([hidden]) [data-inline-mark="bold"]');
    assert.equal(
      await page.$eval('.archive-editor-window:not([hidden]) [data-inline-format-status]', (output) => output.textContent),
      '已切换加粗',
    );
    assert.equal(await page.$eval(field, (control) => Boolean(control.querySelector('strong'))), true);
    assert.equal(
      await page.$eval('[data-archive-editor]', (form) => form.dataset.editorSubmissionState),
      'editing',
    );
  },
);
  ...
  async (t) => {
    ...
    await page.waitForFunction(
      ({ archiveId, amendmentId }) => {
        ...
        return Boolean(
          ...
          && ledger.querySelector(`[data-open-amendment-history="${amendmentId}"]`),
        );
      },
      { timeout: 10_000 },
    ...
    const publishedArchiveUi = await page.$eval(
      ...
      (ledger, amendmentId) => {
        ...
        const historyButton = ledger.querySelector(`[data-open-amendment-history="${amendmentId}"]`);
        return {
          metadata,
          historyVersion: historyButton?.querySelector('b')?.textContent.trim(),
          inlineAmendmentCount: ledger.querySelectorAll('[data-amendment-for]').length,
        };
      },
      amendment.id,
    );
    ...
    assert.ok(
      await page.$eval('[data-amendment-history-modal]', (modal, expected) =>
        modal.textContent.includes(expected), returnedOverview),
      'The history dialog must show the amended field value',
    );
    assert.equal(
      await page.$eval('[data-amendment-history-modal]', (modal) => {
        const dialog = modal.querySelector('[role="dialog"]');
        return dialog?.getAttribute('aria-labelledby') === dialog?.querySelector('h4')?.id;
      }),
      true,
      'The history dialog must name itself with its visible title',
    );
