...
The seventh browser run reported workflow controls at 44×44 but assistant controls at scaled animation dimensions. The test now waits for the workspace assistant's opening animation class to clear before resizing and measuring, so `getBoundingClientRect()` observes the stable interactive target rather than its transient transform.
