  ...
  const listClerkDossierEntries = async (profileId) => {
    ...
    const [contributions, responses, tasks] = await Promise.all([
      unwrap(supabase.from('archive_contributions')
        ...
        .select('id,archive_id,owner_id,template_id,title,kind,status,draft_content,submitted_at,created_at,updated_at,archive:archives(id,code,title,category,visibility),versions:archive_versions!archive_versions_contribution_id_fkey(id,version_label,approved_at,created_at)')
